...
const VideoFile: React.FC<Props> = ({ video }) => {
    ...
    return (
        <div className="flex bg-gray-800 flex-col rounded-md h-full" onClick={() => navigate(`/videos/${video.VideoLink}`)}>
            <video src={video.Video} ref={videoRef} hidden></video>
            <div className='rounded-md bg-gray-800 relative' onMouseOver={() => setState(true)} onMouseOut={() => setState(false)}>
                {!state ? (
                    ...
                    <div className='relative aspect-video'>
                        ...
                        ></video>
                        <div className="absolute bottom-0 left-0 h-1 rounded-r-full bg-red-600" style={{ width: `${watching}%` }}> <button className='absolute -right-2 -top-2.5'><i className='fa fa-circle text-xs text-red-600'></i></button> </div>
                    </div>
                )}
            </div>
            <div className="flex flex-row mt-2 ml-4">
                ...
                <div className="w-[80%]">
                    ...
                    <span className="text-gray-300 font-medium text-xs pl-2">
                        {video.Reactions[0].Views.length} views • {duration}
                    </span>
                </div>
            </div>
            <style>{`
