...
export const useVideoPlayer = () => {
    ...
    useEffect(() => {
        ...
        if (token && VideoLink) {
            dispatch(getVideo({ token, VideoLink })).then((response: any) => {
                ...
                console.log(video.Channel[0].Subsribers.includes(response.payload.user._id), response.payload.user._id)
                setSubscribe(video.Channel[0].Subsribers.includes(response.payload.user._id))
                if (videoRef.current) {
    ...
    const handleUnsubscribe = () => {
        if (auth.user && videoData) {
            socket?.emit("unsubscribeChannel", auth.user._id, videoData.Channel[0]._id)
            setSubs(prev => prev - 1)
            return setSubscribe(false)
        } else return
    }
    return { videoRef, playerRef, loadingVideo, comments, setComments, videoData, handlePlayerClick, dialog, setDialog, handleSubscribe, handleUnsubscribe, subs, subscribe }
}